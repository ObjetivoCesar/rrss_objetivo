...
const EXPERT_METAPHORS: Record<number, { title: string, subtitle: string, scene: string, identity: string, branding: string, style: 'Cyber-Tech' | 'Organic-Premium' }> = {
  1: {
...
function generateUniversalPrompt(id: number, meta: typeof EXPERT_METAPHORS[1]): string {
...
async function masterProcess() {
  ...
  const updatedRecords = records.map((record: any) => {
    ...
    if (record.Contenido_Markdown && record.Contenido_Markdown.trim().startsWith('# ')) {
      const oldContent = record.Contenido_Markdown;
